            ...
            <el-form :inline="true" :model="searchForm" ref="searchForm" size="small" class="demo-form-inline">
                ...
                <el-form-item>
                    <el-button type="primary" size="mini" icon="el-icon-search" @click="handleSearch"
                               v-has="'systemLog:select'" title="查询">
                    </el-button>
                    <el-button size="mini" icon="el-icon-refresh-left" @click="handleReset"
                               v-has="'systemLog:select'" title="重置">
                    </el-button>
                </el-form-item>
            </el-form>
            <el-row>
                <el-button size="mini" type="danger" icon="el-icon-delete" v-has="'systemLog:delete'" @click="handleDeleteBatch">批量删除</el-button>
            </el-row>
            <el-table stripe
                      ref="table"
                ...
                <el-table-column fixed="right" width="70" label="操作">
                    <template slot-scope="scope">
                        <el-button type="danger" size="mini" icon="el-icon-delete" v-has="'systemLog:delete'"
...
<script>
    export default {
        ...
        methods: {
            // 加载表格数据
            loadTable() {
                this.$api.systemLogList(this.searchForm).then(res => {
            ...
            handleDelete(index, row) {
                this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
                    ...
                    this.$api.deleteSystemLog({id: row.id}).then(res => {
                        if (res.data.code == 200) {
            ...
            handleDeleteBatch() {
                ...
                this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
                    ...
                    this.$api.deleteSystemLogByIds({ids: param.join(',')}).then(res => {
                        if (res.data.code == 200) {
                            this.$message.success({
                                message: res.data.message,
                                onClose: () => {
