<template>
    <div class="app-container">
        <el-card>
            ...
            <el-form :model="dataForm" :rules="rules" v-loading="formLoading" ref="dataForm" size="medium" label-width="100px"
                     class="demo-ruleForm">
                <el-row>
                    <el-col :md="12" :sm="24">
...
<script>
    export default {
        ...
        methods: {
            ...
            // 回显数据
            loadPageData() {
                this.$api.getUserInfoById({id: this.$route.query.id}).then(res => {
                    if (res.data.code == 200) {
                        // this.dataForm = res.data.data;
                        let obj = res.data.data;
                        this.dataForm.id = obj.id;
                        this.dataForm.userName = obj.userName;
                        this.dataForm.nickName = obj.nickName;
                        this.dataForm.avatar = obj.avatar;
                        this.dataForm.phone = obj.phone;
                        this.dataForm.sex = String(obj.sex);
                        this.dataForm.description = obj.description;
                        this.dataForm.status = String(obj.status);
                        this.dataForm.roleIds = res.data.data.roleIds.split(',').map(item => {return +item});
            ...
            submitForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        this.$api.updateUserInfo(this.dataForm).then(res => {
                            if (res.data.code == 200) {
